...
The DID pool is a DID numbers range of a specified SIP trunk which is configured in the PBX.

Since BGMconnects is a multi-tenant PBX design for the cloud PBX provider and UCaaS provider. If more than one tenant set up the trunk from the same trunk provider in the PBX, and sets the same DID number for the inbound rule, when a call is arriving at the PBX, the PBX does not know which tenant should route the call to; and when an extension of a tenant makes the call to the trunk, set the outbound caller ID which belongs to another tenant, will cause problems there.

To avoid the problems above, BGMconnects introduced the DID pool concept.
...
The PBX separated the multiple DID/DDI numbers by a semicolon, following formats are allowed:
