...
This article provides solutions for common PBX issues with calls.
...
* If no record of your call appears, it means the PBX did not receive the call. In this case, check if your app or IP phone can reach the PBX.
...
* The PBX can’t provide service at this moment.
...
* The PBX failed to resolve the trunk domain DNS.

### **403 - Forbidden**

This code may appear for the following reasons:

* The current usage exceeds the PBX license.
...
* The trunk received the call from the PBX but returned an error. Please **contact your trunk support team** for further assistance.
...
Sometimes, after configuring the trunk and inbound rule in the PBX, calls to the DID number may not be received. To troubleshoot this issue, please follow the steps below.

### Check the Trunk IP Address

If the trunk sends calls to the PBX from an IP address that does not match the IP host or the **Associated IP Addresses** configured for the trunk in the PBX, the PBX will respond with a 407 error.

It’s important to contact your trunk provider to obtain the IP addresses they use to send the INVITE messages to the PBX. Once you have this information, add the IP addresses to the **Trunk Associated IP Addresses** as shown in the screenshot below.
...
When the PBX identifies that the call is coming from a configured trunk, it will check if the dialed number falls within the trunk's DID pool range. If the dialed number is not within the DID pool for a tenant’s configured trunk, the PBX will respond with a **407** error to the trunk.

In many scenarios, customers configure the DID pool without including the **country code**, but the trunk sends the INVITE message to the PBX with the country code prefixed. For example, if a tenant’s DID pool is set to **620012861** or **620012861-6200128618**, but the trunk sends the DID as **33620012861** or **+33620012861**, the PBX will return a 407 error because the dialed number is outside the DID pool range. In this case, the DID pool should be configured as **33620012861** or **33620012861-633200128618**.

**Note:** You do not need to include the "+" prefix in the DID pool configuration; the PBX will automatically handle this scenario.

### Check the Inbound Rules

Once the PBX verifies that the call is coming from the trunk, it will proceed to match the tenant's inbound rules. You must ensure that the dialed number matches at least one inbound rule.
...
Typically, the **Caller Number Mask** field does not need to be filled in. For more details, please refer to the guide on [**Configuring Inbound Rules**](../bgmconnects-pbx-administration-guide/8-call-route-management/configuring-inbound-rule.md).
...
In some cases, the trunk may not respond to the PBX when it sends REGISTER, INVITE, or other SIP messages. To troubleshoot this issue, please follow the steps below.

### Check the Whitelist on the Trunk Side

Some trunk providers require you to whitelist the PBX’s IP address. Please contact your trunk provider to confirm this requirement.

### Change the User-Agent

Some trunk providers host their services based on BGMconnects' competitors' platforms, and they may block the PBX **User-Agent**.  So if the trunk recognizes that the SIP message’s **User-Agent** header includes the "**BGMconnects**" string, it may silently discard the message without sending any response to the PBX.

To resolve this, sign in to the PBX web portal as a **System Administrator**, navigate to the menu **Advanced > Settings**, change the **User-Agent**, save the changes, and then try again.

**Note:** This is an unfortunate behavior that wastes both customer and support team time during troubleshooting.

<figure><img src="../../.gitbook/assets/bgmconnects-pbx-user-agent.png" alt=""><figcaption></figcaption></figure>
...
1.  In the trunk **Options** section of the PBX, there is a setting labeled **"Use the private IP address to communicate with this trunk."** This setting determines how the PBX populates the media IP address in the **SDP** of **INVITE** and **200 OK** messages during call establishment.
...
3. If your PBX is hosted on a cloud platform such as **AWS, Microsoft Azure, or Google Cloud (GCE)**, you must configure network security rules to allow media traffic.
...
By following these steps, you can resolve most voice-related issues with trunk calls. If problems persist, verify the SIP trunk provider's settings and contact the [BGMconnects support team](mailto:support@bgmconnects.com).

## 5. Troubleshooting PBX Web Portal Login Failures

Sometimes, you may encounter error messages when trying to log in to the PBX Web Portal. Refer to the following common errors and their potential causes:
...
* **Error: Unknown Error (500)**
  ...
  * As a temporary solution, you can free space by deleting archived log files with the `.gz` extension in the `/var/lib/bgmconnects/pbx/log` directory.
  * For a permanent solution, consider increasing your disk size.
  * To receive alerts before running out of space, enable email notifications by referring to the [Configuring Email Notifications](../bgmconnects-pbx-administration-guide/configuring-email-notifications.md) guide.
...
* **Error: Server error, please try again later (502)**
  * This indicates that the PBX API gateway is down.
  * Insufficient disk space can also trigger this error.
  * If resolving disk space issues does not help, please contact the Support Team.
* **Error: the client IP address x.x.x.x has been blocked until \[date/time] (DENIED)**
  ...
  * Use a different IP address (or network) to access the PBX Web Portal with the correct credentials.
  * Remove the blocked IP address from the **IP Blacklist** once you regain access.

Following these steps should help you diagnose and resolve the most common login issues with the PBX Web Portal. If you continue to experience problems, please contact BGMconnects Support for further assistance.

## 6. Collect the Logs

BGMconnects provides a powerful tool that allows you to troubleshoot calls visually. For more details, please refer to the article [Trace Server - A Better Way to Monitoring SIP Messages and QoS for BGMconnects.](../../pbx_v12/tutorials/debug-sip-message.md)&#x20;
...
5. Send the captured **PCAP** file to the BGMconnects team.
...
4. Save the captured packets as a **.pcapng** file and send it to the BGMconnects team.

### Download BGMconnects Log Files

By default, the BGMconnects PBX log files are stored in the following locations:

#### Linux

```sh
/var/lib/bgmconnects/pbx/log/
```

If you configured a different path rather than the default path during the installation, the log files will be located in:

```sh
path/bgmconnects/pbx/log/
```

#### Windows

```
C:\ProgramData\BGMconnects\pbx\log\
```
...
Typically, you should copy the **callmanager.log** file, compress it, and send it to the BGMconnects team for further investigation.

Feel free to contact the BGMconnects support team at [support@bgmconnects.com](mailto:support@bgmconnects.com). Our team will help you resolve any issues.
