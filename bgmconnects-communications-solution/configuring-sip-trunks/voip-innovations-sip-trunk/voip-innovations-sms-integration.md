...
>   you must verify that the **10DLC registration process** has been completed with your carrier **before** attempting to use SMS in the PBX.
...
The PBX requires API credentials from VoIP Innovations to enable SMS functionality.
...
4. Complete the required fields:
   * **Username**\
     This value serves as the **API Key** and will be used later in the PBX.
   * **Password**\
     This value serves as the **API Secret** and will also be required later.
   * **Server IP**\
     Enter the **public IP address** of your PBX server.
5. Click **Add API Login** to save the configuration.

**Expected Result**

* An API user is created and ready to be used by the PBX for SMS integration.
...
### Configure SMS with VoIP Innovations Trunk in the PBX

This section explains how to configure SMS/MMS messaging in the PBX using a VoIP Innovations SIP trunk.
...
> **Note**\
> For more information about tenant roles and permissions, see [Tenant Management](../../bgmconnects-pbx-administration-guide/3-tenant-management/).

***

### Add an SMS Configuration in the PBX

This section explains how to create an SMS configuration in the PBX and link it to a VoIP Innovations SMS-enabled DID.
...
Each SMS-enabled DID must be associated with the PBX webhook on the VoIP Innovations side.
...
5. In the **URL** field, paste the **Webhook URL** copied from the PBX.
...
You can now [create outbound and inbound rules](configuring-outbound-and-inbound-calls.md) in the PBX to send and receive SMS/MMS messages using the VoIP Innovations trunk, just as you would configure rules for outbound and inbound voice calls.
