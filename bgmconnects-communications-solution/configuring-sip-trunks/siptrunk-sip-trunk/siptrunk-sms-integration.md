...
To integrate the SIPTRUNK SMS with the PBX, you must obtain an **Access Key** and **Secret Key**, which are used to authenticate communication between the PBX and the SIPTRUNK platform.
...
3. Copy the **Access Key** and **Secret Key** and keep it available for later configuration in the PBX.

The **Access Key** and **Secret Key** are required to securely authenticate API requests between the **PBX** and **SIPTRUNK**.

***

### Configure SMS with a SIPTRUNK Trunk in the PBX

Before configuring SMS in the PBX, ensure that you have already configured a SIPTRUNK trunk using one of the following guides:
...
You can sign in to the PBX Web Portal using one of the following methods:
...
For more information, refer to [Tenant Management](../../bgmconnects-pbx-administration-guide/3-tenant-management/).
...
4. Configure the following settings:
   ...
   * **Sender ID**
     ...
     * If left empty, the PBX will use the **DID number associated with the SIPTRUNK  trunk** as the Sender ID.
...
This webhook enables SIPTRUNK to deliver inbound SMS events to the PBX.
...
* [Create Outbound Rules and Inbound Rules](configuring-outbound-and-inbound-calls.md) in the PBX for sending and receiving SMS/MMS using the **SIPTRUNK** trunk.
