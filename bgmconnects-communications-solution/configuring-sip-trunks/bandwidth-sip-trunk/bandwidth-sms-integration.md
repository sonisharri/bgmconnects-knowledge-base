...
### Configure SMS with Bandwidth Trunk in the PBX
...
* Sign in directly as a **Tenant Admin**. For more details, see [Tenant Management](../../bgmconnects-pbx-administration-guide/3-tenant-management/).
...
4. Replace the existing (fake) **Callback URL** with the **Webhook URL** copied from the PBX.
...
You can now [create outbound and inbound rules](configuring-outbound-and-inbound-calls.md) in the PBX to send and receive SMS/MMS messages using the Bandwidth trunk, just as you would configure rules for voice calls.
