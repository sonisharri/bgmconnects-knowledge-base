...
By default, the PBX automatically determines the outbound Caller ID based on the system configuration.\
...
The PBX provides flexible Caller ID controls that allow customization:
...
A user’s outbound Caller ID can be configured in multiple locations. When multiple settings exist, **the PBX applies them using the following priority order (highest to lowest):**
...
Caller ID can be defined globally in the PBX web portal under: **Company > Outbound Caller ID**
...
* **BGMconnects mobile apps** ([iOS ](https://www.bgmconnects.com/downloads/)and [Android](https://www.bgmconnects.com/downloads/)).
...
1. Open the **Contact Details** view in the **BGMconnects** app. The contact’s information and available actions are displayed.
...
1. Open the **SMS or WhatsApp conversation thread** in the **BGMconnects** app. The conversation history and call options are displayed.
...
In some situations, you may want to hide your caller ID when placing a call. BGMconnects allows you to block your caller ID from multiple call entry points.

***

#### Block Caller ID from the Keypad

1. Open the **Keypad** in the BGMconnects app.
...
For details on how the PBX selects the outbound caller ID, see [Handle Outbound Calls Through SIP Trunk](../../bgmconnects-communications-solution/bgmconnects-pbx-administration-guide/7-trunk-management/handle-outbound-calls-through-sip-trunk.md#outboundcallerid).
