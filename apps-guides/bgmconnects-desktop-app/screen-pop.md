...
Screen pop, also known as _call pop_, is a feature in the BGMconnects App that automatically opens a web page when you make or receive a call. This allows agents to quickly view contact information before or during a conversation.
...
* **Embedded window**\
  The screen pop opens inside the BGMconnects App in an embedded browser window, allowing agents to stay within the application while viewing contact information.
* **Default browser**\
...
When BGMconnects launches the URL, the following placeholders are automatically replaced with call-specific values:
...
BGMconnects also supports the following additional parameters:
